...
if(isset($_POST['btneprod']) && ($_POST['btneprod'] != "") && isset($_POST['txtsku']) && ($_POST['txtsku'] != "") && isset($_POST['txtcde']) && ($_POST['txtcde']!= "") && isset($_POST['txtname']) && ($_POST['txtname'] != "") && isset($_POST['txtcstprc']) && ($_POST['txtcstprc'] != "") && isset($_POST['txtsleprc']) && ($_POST['txtsleprc'] != "") && isset($_POST['txtprior']) && ($_POST['txtprior'] != "") && isset($_POST['txtwrnty']) && ($_POST['txtwrnty'] != "") && isset($_POST['txtrtng']) && ($_POST['txtrtng'] != ""))
{
  ...
  $seoh2_tle = glb_func_chkvl($_POST['txtseoh2tle']);
	...
	if($cntprodm < 1)
	{
		...
		$del_prod_strs1 = "DELETE from prod_features_dtl where prods_fetrs_prodm_id = $id";
		if (mysqli_query($conn,$del_prod_strs1))
		{
			$fet_cnt = count($ckhfetrs);
	  for ($fet = 0; $fet < $fet_cnt; $fet++)
	  {
		// echo "here2:".$prod_id."-".$vehtyp."-".$chkloc[$store]."<br>";
		$iqryprod_str_dtl1 = "INSERT into prod_features_dtl (prods_fetrs_prodm_id, prods_fetrs_store_id, prods_fetrs_sts, prods_fetrs_crton, prods_fetrs_crtby) values ('$id','$ckhfetrs[$fet]','$sts','$curdt','$ses_admin')";
		$irsprod_str_dtl1 = mysqli_query($conn,$iqryprod_str_dtl1) or die(mysqli_error());
	  }
	}
