				...
				<div class="row justify-content-center align-items-center">
					...
					<div class="col-md-12">
						<div class="row mb-2 mt-2">
							...
							<div class="col-sm-9">
								<?php
							 $sqry_fet = "SELECT prodfetrm_name, prodfetrm_id FROM prodfetr_mst WHERE prodfetrm_sts ='a' group by prodfetrm_id order by prodfetrm_name";
								$srsfet = mysqli_query($conn,$sqry_fet);
								$num_rows1 = mysqli_num_rows($srsfet);
								while ($srowloc = mysqli_fetch_assoc($srsfet))
								{
									$fetids = '';
									$fetid= $srowloc['prodfetrm_id'];
										$fetnm = $srowloc['prodfetrm_name'];
									$sqryprod_dtl1 = "SELECT prods_fetrs_store_id
						      	from prod_features_dtl where prods_fetrs_prodm_id ='$id'";
						      $srsprod_dtl1 = mysqli_query($conn,$sqryprod_dtl1);
						      while($rowprod_dtl1=mysqli_fetch_assoc($srsprod_dtl1))
						      {
						      	$fetids.=",".$rowprod_dtl1['prods_fetrs_store_id'];
						      }
						      $fetids1=substr($fetids,1);
									$fetid1=explode(',',$fetids1);
									// echo $cstmrid."<br>";
									// print_r($cstmid1);
									$cntid=count($fetid1);
									if (in_array($fetid, $fetid1))
									{
										$slctchk1 = "checked";
									}
									else
									...
									// echo $slctchk;
									?>
									<input name="ckhfetrs[]" type="checkbox" id="ckhloc<?php echo $fetid; ?>" <?php echo $slctchk1; ?> value="<?php echo $fetid; ?>">&nbsp;<?php echo $fetnm; ?>
									<span id="errorsDiv_ckhfetrs"></span>
									<?php
								}
								?>
							</div>
